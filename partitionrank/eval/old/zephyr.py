...
def score_zephyr(dataset : str, 
    ...
    del queries
    out_file = join(output_path, f"zephyr.{mode}.{buffer}.{window_size}.{stride}.{depth}.tsv.gz")
    if os.path.exists(out_file): 
        logging.info(f"Skipping zephyr.{mode}.{buffer}.{window_size}.{stride}.{depth}, already exists")
        return
    log_file = join(output_path, f"zephyr.{mode}.{buffer}.{window_size}.{stride}.{depth}.log")
    ...
    model = RankZephyr(device='cuda', n_gpu=n_gpu, mode=mode, window_size=window_size, buffer=buffer, stride=stride, max_iters=max_iters, depth=depth)
